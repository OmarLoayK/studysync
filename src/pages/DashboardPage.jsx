...
const emptyTask = { title: "", course: "", description: "", dueDate: "", priority: "Medium", estimatedMinutes: 60, proofLink: "", completionNote: "" };
...
function TaskForm({ form, setForm, onSubmit, onCancel, busy, editing }) {
  return (
    <form className="grid gap-4 md:grid-cols-2" onSubmit={onSubmit}>
      ...
      <TextInput label="Proof link" placeholder="https://..." value={form.proofLink} onChange={(event) => setForm((current) => ({ ...current, proofLink: event.target.value }))} hint="Optional now, but required if you want to complete without a note." />
...
function TaskCard({ task, onEdit, onDelete, onToggleComplete }) {
  ...
  const canComplete = Boolean(task.proofLink?.trim() || task.completionNote?.trim());
...
export default function DashboardPage() {
  ...
  async function handleToggleComplete(task) {
    ...
    if (!task.completed && !hasProof) {
      setError("Add a proof link or completion note before marking a task complete.");
      return;
    }
    await updateTask(user.uid, task.id, { completed: !task.completed, completedAt: task.completed ? "" : new Date().toISOString() });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <Card>
          ...
          <div className="mt-6 space-y-5">
            ...
            <ProgressBar label="Current monthly AI credits" value={profile?.usage?.aiGenerationsUsed ?? 0} max={profile?.usage?.aiGenerationsLimit ?? 60} helper={profile?.plan?.tier === "premium" ? "Premium credits reset monthly." : "Upgrade to premium to activate credits."} />
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-gradient-to-br from-slate-900 via-slate-900 to-sky-950">
            ...
            <p className="mt-3 text-slate-300">Premium users can generate schedules, quizzes, flashcards, explainers, and breakdowns directly from the AI tools workspace.</p>
      ...
      <Modal open={taskModalOpen} title={editingTask ? "Edit task" : "Create a new task"} description="Capture the task cleanly so the dashboard can sort, badge, and track it properly." onClose={() => setTaskModalOpen(false)}>
        <TaskForm form={taskForm} setForm={setTaskForm} onSubmit={handleTaskSubmit} onCancel={() => setTaskModalOpen(false)} busy={formBusy} editing={Boolean(editingTask)} />
      </Modal>
    </div>
  );
}
